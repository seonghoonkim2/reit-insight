...
## 0단계 — POC (완료) ✅
- `collect.py`: OpenDART 자동 수집 + **정정→최신본 선택**(`filing_group_key`/`is_latest_version`) + 섹션 경로 + **SQLite 저장**(`db.py`)
- `summarize.py`: Claude API로 **요약·핵심지표 자동 추출**(원문 기반, 출처 표기)
- `web/index.html`: **공시렌즈 앱** — 홈/검색/회사/보고서/토픽, **문단 단위 검색** + 연도/섹션/숫자 필터,
  보고서 내 검색, 정정 배지, **광고 노출 규칙(`canShowAds`)** + 면책문구, 라이트 리서치 테마
- 남은 한계: 데이터가 `data.js` 한 파일(소수 기업), 검색은 클라이언트 문자열 매칭(아직 검색엔진 아님), SEO 정적페이지/sitemap 미발행
