...
현재 위치: **0단계(POC) 완료** — 수집 스크립트 + 검색 화면(데모)까지 동작.
아래는 "취미 수준 → 실제 서비스"로 키우는 단계별 계획입니다. 입문자 기준으로 작게 쪼갰어요.

---

## 0단계 — POC (지금 여기) ✅
- `collect.py`: OpenDART API로 기업 몇 곳의 사업보고서 자동 수집 → 텍스트/섹션 추출
- `web/index.html`: 검색·강조·상세보기 + 부가가치(요약/지표) 자리
- 한계: 데이터가 브라우저용 `data.js` 한 파일에 들어감(소수 기업만 가능), 검색은 단순 문자열 매칭
