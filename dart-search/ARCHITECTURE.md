...
> **방향(2026-06 업데이트)**: GoInsider처럼 **상장리츠(REITs)** 를 깊게 보여주는 정보 플랫폼이 최종 목표.
> 메인 = 리츠(핵심지표·배당·NAV·포트폴리오·신용등급·AI요약), 부가 = 공시(DART 사업보고서) 전문검색,
> 리츠 발행 채권 = 별도 페이지(ISIN별). 아래 문서의 "공시렌즈" 검색엔진은 이 플랫폼의 한 축입니다.
> 리츠/채권은 db `reits`/`bonds` 테이블 + SPA(#/reits·#/reit·#/bonds·#/bond) + FastAPI(/api/v1/reits·reit·bonds·bond) + Next 페이지로 구현됨.

---

# (이하: 공시 검색엔진 설계 — 플랫폼의 한 축)
