# 리츠인사이트 — 상장리츠(REITs) 정보 플랫폼

> 방향: **GoInsider처럼 상장리츠를 깊게** 보여주는 정보 플랫폼.
> 메인 = **상장리츠**(핵심지표·배당·NAV·포트폴리오·신용등급·AI 요약), 부가 = **공시(DART 사업보고서) 전문검색**,
> 그리고 **리츠가 발행한 채권**은 별도 페이지(ISIN별 상세).

상장리츠를 핵심지표·배당·포트폴리오·AI 요약으로 정리하고, 각 리츠의 **공시(사업보고서)** 와
**발행 채권**까지 연결합니다. 공시 검색 엔진(구 "공시렌즈")은 이 플랫폼의 한 축입니다.
