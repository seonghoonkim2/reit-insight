...
export default {
  async fetch(request, env) {
    ...
    // Cloudflare는 Worker에 넘기는 URL scheme과 CF-Visitor를 https로 정규화할 수 있어,
    // 실제 클라이언트 연결의 TLS 메타데이터도 함께 본다. 프로덕션 request.cf는 있는데
    // tlsVersion이 없으면 클라이언트가 평문 HTTP로 들어온 요청이다.
    let cfVisitor = {};
    ...
    if (url.protocol === "http:" || cfVisitor.scheme === "http" || (request.cf && request.cf.httpProtocol && !request.cf.tlsVersion)) {
      url.protocol = "https:";
